...
SCHOOL_YEAR = {
    'Năm thứ nhất': '1', 'Năm thứ hai': '2', 'Năm thứ ba': '3','Năm thứ tư': '4', 'Năm thứ 5': '5', 'Học sinh THPT': 'THPT' , 'Khác': '6'
}
...
def simplify(record, semester):
    ...
    new_record[7] = SCHOOL_CODE_ALL.get(record[8], 'UNK')
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) <3):
        ...
        # do not add if there is no motivation letter
            if (data[i][89].strip() == "" and data[i][90].strip() == ""):
                res = simplify(data[i], semester)
                newdata.append(res)
    else:
        for i in range(1, len(data)):
        # do not add if there is no motivation letter
            if not (data[i][89].strip() == "" and data[i][90].strip() == ""):
                res = simplify(data[i], semester)
                newdata.append(res)   
